...
class Signup12 extends StatefulWidget {
  ...
  final String email;
  final String role;
  const Signup12({super.key, required this.uid, required this.email, required this.role});
...
class _Signup12State extends State<Signup12> {
  ...
  Future<void> _next() async {
    if (descriptionController.text.trim().isEmpty) return showSnack(context, "Please enter your description.");
  ...
  void _back() => Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => Signup11(uid: widget.uid, email: widget.email, role: widget.role)));

  @override
  void dispose() { descriptionController.dispose(); super.dispose(); }
    ...
    return Stack(children: [
      Scaffold(appBar: signupAppBar(), body: SingleChildScrollView(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Image.asset("assets/images/doctor.png", width: 349, height: 260),
          Text("Doctor Description", style: GoogleFonts.poppins(fontSize: 23, fontWeight: FontWeight.w700, color: kPink)),
          const SizedBox(height: 15),
          Text("Tell patients about yourself", style: GoogleFonts.poppins(color: kBlue, fontWeight: FontWeight.w700, fontSize: 18)),
          TextField(controller: descriptionController, minLines: 6, maxLines: 8, decoration: signupInputDecoration(hint: "Write your expertise, specialization, and clinic details...")),
